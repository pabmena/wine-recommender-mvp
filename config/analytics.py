...
# En Vercel (entorno serverless) la única carpeta escribible es /tmp
...
def init_analytics_db():
    ...
    try:
        ...
        print(f"Advertencia: No se pudo inicializar la base de datos de analíticas ({e})")

def track_event(client_id, event_type, session_id, payload=None):
    """
    Registra un evento de tracking en la base de datos. Degrada de forma segura si falla.
    """
    try:
        init_analytics_db()
        conn = sqlite3.connect(DATABASE_PATH, timeout=10)
        ...
        print(f"Advertencia: No se pudo registrar el evento {event_type} para el cliente {client_id} ({e})")

def get_client_dashboard_metrics(client_id):
    """
    Obtiene las métricas agregadas del dashboard para un cliente. Retorna métricas demo si falla.
    """
    default_metrics = {
        'quizzes_started': 0,
        'quizzes_completed': 0,
    ...
    try:
        ...
        print(f"Error al leer las métricas de analíticas ({e})")
        return default_metrics
